...
| 層 | 担当 | 範囲 |
|---|---|---|
| **Layer A** | GAS（本リポジトリの `gas/`） | 余白・行間・字下げ・フォント・色・コントラスト比・強調比率・絵文字検出・表スタイル・ページ番号・作成日記載・URL 形式・読点数・漢字連続 — 決定論で答えが一意に決まるもの |
| **Layer B** | LLM CLI（公開予定） | 一文長・受動態・二重否定・曖昧副詞・トーン・用語統一 — 意味判断を要するもの |
...
**雛形のみ**。`gas/handlers/*.gs` の各関数は TODO Finding を返すスケルトン状態です。loader → checker → 各 handler → report のパイプラインは通りますが、実際の判定ロジックは各ハンドラごとに順次実装中。

- 実装優先度は [rules-matrix.md](rules-matrix.md) 末尾「Layer A 実装優先度」を参照。
- Layer B（LLM）の CLI / GitHub Action は別フェーズで着手予定。
